...
import { Icon, useColorModeValue, HStack, Tag, TagLabel, TagRightIcon, Progress } from "@chakra-ui/react";
...
export const Temperature: React.FC = () => {
    ...
    return (
        <HStack>
            ...
            <Progress
                colorScheme={color.replace(/\.(.+)/g, "")}
                hasStripe
                value={iteration}
                height={35}
                w={35}
                min={temp.initialTemp}
                max={temp.explodeTemp}
                style={{
                    borderRadius: 5,
